...
export const getResourceMints = async (): Promise<ResourceMints> => {
  try {
    const response: AxiosResponse<ApiResponse<ResourceMintsRaw>> = await axios.get<
      ApiResponse<ResourceMintsRaw>
    >("https://n8n.staratlasitalia.com/webhook/getResourceMints");
    if (response.data.code !== 200) throw new GetResourceMintsError(response.data.error);
